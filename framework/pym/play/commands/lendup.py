...
COMMANDS = ['parallel-autotest','parallel-autotest-precompile']

HELP = {
    'parallel-autotest': "Automatically run all application tests in parallel",
    'parallel-autotest-precompile': "Automatically run all application tests in parallel using a precompiled test server",
}
...
def execute(**kargs):
    ...
    if command == 'parallel-autotest':
        parallel_autotest(app, args)
    if command == 'parallel-autotest-precompile':
        parallel_autotest_precompile(app, args)


def parallel_autotest(app, args):
    app.check()
    ...
    fpcp = [os.path.join(app.play_env["basedir"], 'modules/testrunner-lendup/lib/play-testrunner.jar')]
    fpcp_libs = os.path.join(app.play_env["basedir"], 'modules/testrunner-lendup/firephoque')
    ...
    if os.name == 'nt':
        cp_args = ';'.join(fpcp)
    threadpool_size = 6;
    if app.readConf('testrunner.threadpool'):
        threadpool_size = app.readConf('testrunner.threadpool')

    java_cmd = [app.java_path(), '-classpath', cp_args, '-Dthreadpool.size=%s' % (threadpool_size), '-Dapplication.url=%s://localhost:%s' % (protocol, http_port), '-DheadlessBrowser=%s' % (headless_browser), 'play.modules.testrunner.FirePhoque']
    if protocol == 'https':
...
def parallel_autotest_precompile(app, args):
    app.check()
    print "~ Running in test mode"
    print "~ Ctrl+C to stop"
    print "~ "
